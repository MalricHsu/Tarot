...
import { buildClarificationFallback, buildInterpretation, defaultSpread, getSpreadById, spreads } from './logic/spread';
import type { DrawnCard, Orientation, ReadingResult, SpreadDefinition, SpreadId, TarotCard } from './types';
...
function Clock() {
  const time = useClock();
  return <time className="live-clock" aria-label="目前時間">{time}</time>;
}
...
function selectedSlotsToDrawnCards(selectedSlots: ChoiceSlot[], spread: SpreadDefinition): DrawnCard[] {
...
function buildShareText(question: string, spread: SpreadDefinition, reading: ReadingResult): string {
...
function App() {
  ...
  useEffect(() => {
    ...
    const readingTimer = window.setTimeout(() => setPhase('reading'), 720 + drawnCards.length * 520);
  ...
  useEffect(() => {
    if (phase !== 'reading' || readingStartedRef.current || drawnCards.length !== cardsToChoose) return;
    ...
    async function readCards() {
      try {
        ...
        console.error(error);
        setReading(buildInterpretation(trimmedQuestion, drawnCards, selectedSpread));
  ...
  useEffect(() => {
    if (phase === 'reading' || phase === 'done') {
      ...
      window.setTimeout(() => el?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 250);
    }
  }, [phase]);
  ...
  const chooseCard = (slotId: string) => {
    if (phase !== 'choosing' || selectedIds.includes(slotId) || selectedIds.length >= cardsToChoose) return;
  ...
  const requestClarification = async () => {
    ...
    try {
      ...
      console.error(error);
      setClarification(
        buildClarificationFallback(trimmedQuestion, reading.cards, selectedSpread, {
          interpretations: reading.interpretations,
          summary: reading.summary,
        }),
      );
      setClarificationError('Gemini 賢者解析暫時無法取得，已改用本地解析補上。');
    } finally {
      setIsClarifying(false);
    }
  };
  ...
  const downloadShareImage = () => {
    ...
    const textLines = wrapCanvasText(context, buildShareText(trimmedQuestion, selectedSpread, reading), contentWidth);
    ...
    const glow = context.createRadialGradient(width * 0.18, 100, 20, width * 0.18, 100, 380);
    ...
    <>
      <main className="app-shell">
        <section className="ritual-stage" aria-labelledby="app-title">
          <section className={`hero-table phase-${phase}`} aria-label="燭光塔羅桌">
            <div className="ritual-panel">
              ...
              <section className="question-orb" aria-label="輸入問題">
                <form className="question-form" onSubmit={beginRitual}>
                  <fieldset className="spread-picker" disabled={phase !== 'idle'}>
                    <legend>選擇牌陣</legend>
                  ...
                  {phase === 'idle' ? (
                    <button className="primary-action" type="submit" disabled={!canBegin}>
                      <Flame size={16} />
                      抽取命運之牌
                    </button>
                  ) : (
                    <div className="action-row">
                      ...
                      <button
                        ...
                        disabled={phase === 'focusing' || phase === 'revealing' || phase === 'reading'}
                      >
                        <RotateCcw size={15} />
                        重新聆聽命運
                      </button>
          ...
          <section className="deck-area" aria-label="塔羅牌區">
            ...
            {drawnCards.length === cardsToChoose && ['revealing', 'reading', 'done'].includes(phase) ? (
              <div id="reading-results">
                <div className="section-divider">
                  <span>{phase === 'revealing' ? '牌面展開' : `${selectedSpread.label}解讀結果`}</span>
                </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="card-copy">
                          ...
                          {phase === 'reading' ? <p className="loading-text">正在解讀牌面⋯⋯</p> : null}
                          {phase === 'done' ? (
                            <p className="card-interpretation">{reading?.interpretations[index]}</p>
                          ) : null}
                        </div>
                      </article>
                    );
          ...
          {phase === 'reading' ? (
            <aside className="summary loading" aria-label="總結建議" aria-live="polite">
              <h2>正在解讀牌面</h2>
              <p>牌意正在匯整，請稍候片刻。</p>
            </aside>
          ) : null}
          ...
          {phase === 'done' && reading ? (
            <aside className="summary" aria-label="總結建議">
              ...
              <div className="result-actions" aria-label="結果操作">
                <button className="secondary-action icon-action" type="button" onClick={copyShareText}>
                  <Copy size={16} />
                  複製文字
                </button>
                <button className="secondary-action icon-action" type="button" onClick={downloadShareImage}>
                  <Download size={16} />
                  下載分享圖
                </button>
              </div>
              {shareStatus ? <p className="status-text" role="status">{shareStatus}</p> : null}
              <div className="clarification-panel">
                ...
                {isClarifying ? <p className="loading-text">賢者解析中⋯⋯</p> : null}
                {clarificationError ? <p className="field-error">{clarificationError}</p> : null}
                {clarification ? (
